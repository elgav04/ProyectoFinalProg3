...
import { tipovehiculos} from 'src/app/interfaces/user.interface';
...
export class TipovehiculosComponent implements OnInit {
  ...
  getUser() {
    this.Data.getAll('/tipovehiculos')
      .subscribe(res => {
          this.TUser = res;
  ...
  AgregarValor(){
    ...
    delete this.user.ctipov;   
    this.Data.save(this.user,'/tipovehiculos')
        .subscribe(
          res => {
          this.getUser();
  ...
  EliminarData(id: number){
    this.Data.delete(id, '/tipovehiculos')
      .subscribe(
        res => {
          this.getUser();
